...
#property link      "mailto: ashokjjo@yahoo.com"
...
double breakout[];   // breakout
double sqColor[];   // breakout
...
//======================
   ...
   SetIndexBuffer(3,breakout);
   SetIndexStyle(4,DRAW_FILLING, STYLE_SOLID);
   SetIndexBuffer(4,sqColor);
   ...
   for(int i=limit; i>=0; i--) //main indicator FOR loop
     {
      ...
      if(Boll_Upper_Band>=Kelt_Upper_Band || Boll_Lower_Band<=Kelt_Lower_Band)
        ...
        sqzOn = false; sqzOff = false;
        if(Boll_Lower_Band>Kelt_Lower_Band && Boll_Upper_Band<Kelt_Upper_Band) {
         sqzOn = true;
         sqColor[i] = 2; // black
        }
        ...
        if(sqzOn==false && sqzOff==false) {
         noSqz = true;
         sqColor[i] = 1; // blue
        }
        
        //Print("sqColor:" + sqColor[i]);
        
        //color scolor = noSqz ? Blue : sqzOn ? Black : Gray;
        //scolor = Yellow;
        //Print("Color:"+ scolor);
        
        //PlotIndexSetInteger(0,PLOT_LINE_COLOR,0,scolor); 
        //indicator_color5 scolor;
      ...
      if(Squeeze[i]==1 && Squeeze[i+1]==0 && Momentum[i]>0 && i==0) // a cross above zero line and Mom > 0
        {
         if(Last_Alert_Time!=Time[0])
           {
            //if(Alert_On==true) Alert("Alert: Possible Breakout - "+Symbol()+" - "+TimeToStr(TimeLocal())+","+Ask);
            Last_Alert_Time=Time[0];
            breakout[i] = Ask;
            Print("BREAKOUT: " + breakout[i]);
      ...
      if(Squeeze[i]==1 && Squeeze[i+1]==0 && Momentum[i]<0 && i==0) // a cross above zero line and Mom < 0
        {
         if(Last_Alert_Time!=Time[0])
           {
            //if(Alert_On==true) Alert("Alert: Possible Breakout - "+Symbol()+" - "+TimeToStr(TimeLocal())+","+Bid);
            Last_Alert_Time=Time[0];
            breakout[i] = Bid;
            Print("BREAKOUT: " + breakout[i]);
